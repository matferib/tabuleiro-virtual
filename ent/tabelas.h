...
class Tabelas : public ntf::Receptor {
 public:
  ...
  const ItemTesouroProto& Pocao(const std::string& id) const;
  const ItemTesouroProto& PergaminhoArcano(const std::string& id) const;
  const ItemTesouroProto& PergaminhoDivino(const std::string& id) const;
  const ItemTesouroProto& Varinha(const std::string& id) const;
  const ItemTesouroProto& Anel(const std::string& id) const;
  const ItemTesouroProto& Manto(const std::string& id) const;
  const ItemTesouroProto& Luvas(const std::string& id) const;
  const ItemTesouroProto& Amuleto(const std::string& id) const;
  const ItemTesouroProto& Chapeu(const std::string& id) const;
  const ItemTesouroProto& Bracadeiras(const std::string& id) const;
  const ItemTesouroProto& Botas(const std::string& id) const;
  const ItemTesouroProto& ItemMundano(const std::string& id) const;
  const ItemTesouroProto& ItemMaravilhoso(const std::string& id) const;
  ...
  const TemplateTerreno& Terreno(const std::string& id) const;
 ...
 private:
  ...
  std::unordered_map<std::string, const ItemTesouroProto*> pocoes_;
  // Pergaminhos sao gerados, entao nao tem uma tabela pra apontar (a tabela eh so pra merge).
  std::unordered_map<std::string, ItemTesouroProto> pergaminhos_arcanos_;
  std::unordered_map<std::string, ItemTesouroProto> pergaminhos_divinos_;
  std::unordered_map<std::string, const ItemTesouroProto*> aneis_;
  std::unordered_map<std::string, const ItemTesouroProto*> itens_mundanos_;
  std::unordered_map<std::string, const ItemTesouroProto*> itens_maravilhosos_;
  std::unordered_map<std::string, const ItemTesouroProto*> varinhas_;
  //std::unordered_map<std::string, const Municao*> municoes_;
  std::unordered_map<std::string, const ItemTesouroProto*> mantos_;
  std::unordered_map<std::string, const ItemTesouroProto*> luvas_;
  std::unordered_map<std::string, const ItemTesouroProto*> bracadeiras_;
  std::unordered_map<std::string, const ItemTesouroProto*> amuletos_;
  std::unordered_map<std::string, const ItemTesouroProto*> chapeus_;
  std::unordered_map<std::string, const ItemTesouroProto*> botas_;
  ...
  std::unordered_map<std::string, const TemplateTerreno*> terrenos_;
