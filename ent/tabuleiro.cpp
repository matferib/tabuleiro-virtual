...
void Tabuleiro::ConfiguraOlhar() {
  ...
  if (!parametros_desenho_.desenha_mapa_sombras() &&
    ...
    // Mapa de sombras.
    if (MapeamentoSombraLuzDirecional() && !parametros_desenho_.has_picking_x()) {
  ...
  if (MapeamentoSombraLuzDirecional() && parametros_desenho_.desenha_mapa_sombras()) {
    ConfiguraOlharMapeamentoSombrasLuzDirecional();
    return;
  }
...
int Tabuleiro::Desenha() {
  ...
  if (MapeamentoSombraLuzDirecional() && !modo_debug_) {
    GLint original;
    gl::Le(GL_FRAMEBUFFER_BINDING, &original);
    ParametrosDesenho salva_pd(parametros_desenho_);
...
std::unique_ptr<Entidade> Tabuleiro::CriaUmaEntidadePorNotificacao(
  ...
  if (!Desfazendo()) {
    ...
    if (!notificacao.forcado()) {
      entidade_modelo.set_visivel(!modo_mestre || notificacao.has_id_generico());
...
void Tabuleiro::GeraTerrenoAleatorioNotificando(const std::string& id) {
  ...
  ntf::Notificacao grupo_notificacoes;
  grupo_notificacoes.set_tipo(ntf::TN_GRUPO_NOTIFICACOES);
  EntidadeProto template_entidade;
    ...
    for (int y = 0; y < TamanhoY(); ++y) {
      ...
      for (const auto& item : terreno.items_terreno()) {
        if (Aleatorio() <= item.chance()) {
          ...
          auto* ent_adicionada = n_adicao->mutable_entidade();
          ent_adicionada->CopyFrom(template_entidade);
          ent_adicionada->MergeFrom(item.proto());
          // Leve escala e rotacao.
          Vector3 escala = Aleatorio3();
...
bool MaisLongeOlho(const Vector3& olho_pos, const Entidade* lhs, const Entidade* rhs) {
...
// Assume todos no cenario correto.
bool MaisPertoOlho(const Vector3& olho_pos, const Entidade* lhs, const Entidade* rhs) {
  Vector3 lhs_pos(lhs->X(), lhs->Y(), lhs->Z());
  Vector3 rhs_pos(rhs->X(), rhs->Y(), rhs->Z());
  Vector3 olho_lhs = olho_pos - lhs_pos;
  Vector3 olho_rhs = olho_pos - rhs_pos;
...
Tabuleiro::IdCenarioComFuncaoOrdenacao(const ParametrosDesenho& pd) const {
  ...
  if (pd.has_alfa_translucidos()) {
    funcao = [posicao, id_cenario](const Entidade* lhs, const Entidade* rhs) { return MaisLongeOlho(posicao, lhs, rhs); };
...
void Tabuleiro::OrdenaEntidades(const ParametrosDesenho& pd) {
  ...
  std::sort(entidades_ordenadas_.begin(), entidades_ordenadas_.end(), funcao);
}
...
void Tabuleiro::ReiniciaCamera() {
  ...
  if (proto_.has_camera_inicial()) {
    ...
    olho_.set_rotacao_rad(static_cast<float>(-M_PI / 2.0f));
...
void Tabuleiro::BebeOuAplicaItemMundanoNotificando(unsigned int id_entidade, int indice_antidoto) {
  ...
  std::unique_ptr<ntf::Notificacao> notificacao(new ntf::Notificacao);
  std::tie(e_antes, e_depois) = ent::PreencheNotificacaoEntidade(ntf::TN_ATUALIZAR_PARCIAL_ENTIDADE_NOTIFICANDO_SE_LOCAL, *entidade, notificacao.get());
  const auto& item = tabelas_.ItemMundano(entidade->Proto().tesouro().itens_mundanos(indice_antidoto).id());
  {

    *e_depois->mutable_tesouro()->mutable_itens_mundanos() = entidade->Proto().tesouro().itens_mundanos();
    e_depois->mutable_tesouro()->mutable_itens_mundanos()->DeleteSubrange(indice_antidoto, 1);
    if (e_depois->tesouro().itens_mundanos().empty()) {
      e_depois->mutable_tesouro()->add_itens_mundanos();
    }
    std::vector<int> ids_unicos = IdsUnicosEntidade(*entidade);
