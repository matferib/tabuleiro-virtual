...
lowp vec4 CorLuzDirecional(in lowp vec3 vetor_referencia, in InfoLuzDirecional luz_direcional) {
  highp vec3 direcao_luz = vec3(luz_direcional.pos * (1 / luz_direcional.pos.w));
  // dot(v1 v2) = cos(angulo) * |v1| * |v2|.
  lowp float cos_ref = dot(vetor_referencia, direcao_luz);
  if (gltab_especularidade_ligada) {
    cos_ref = pow(cos_ref, 3);
  }
...
lowp vec4 CorLuzPontual(in bool testa_visibilidade, in lowp vec3 vetor_referencia, in InfoLuzPontual luz) {
  ...
  lowp float atenuacao = gltab_especularidade_ligada
    ? step(tam, luz.atributos.r) * (tam / luz.atributos.r)  // atenuacao linear no raio.
    : (0.5 * step(tam, luz.atributos.r) + 0.5 * step(tam, luz.atributos.r * 2.0));  // sem atenuacao por distancia ate raio, depois metade da atenuacao.
  // dot(v1 v2) = cos(angulo) * |v1| * |v2|.
  lowp float cos_ref = dot(vetor_referencia, normalize(objeto_luz));
  if (gltab_especularidade_ligada) {
    cos_ref = pow(cos_ref, 3);
  }
...
void main() {
  ...
  if (gltab_luz_ambiente.a > 0.0) {
    ...
    lowp mat4 cor_luz = mat4(aplicar_luz_direcional * CorLuzDirecional(vetor_referencia, gltab_luz_direcional),
                             CorLuzPontual(true, vetor_referencia, gltab_luzes[0]),
                             CorLuzPontual(false, vetor_referencia, gltab_luzes[1]),
                             CorLuzPontual(false, vetor_referencia, gltab_luzes[2]));
    lowp mat4 cor_luz_2 = mat4(CorLuzPontual(false, vetor_referencia, gltab_luzes[3]),
                               CorLuzPontual(false, vetor_referencia, gltab_luzes[4]),
                               CorLuzPontual(false, vetor_referencia, gltab_luzes[5]),
                               CorLuzPontual(false, vetor_referencia, gltab_luzes[6]));
