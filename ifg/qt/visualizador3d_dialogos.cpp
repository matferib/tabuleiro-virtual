...
void PreencheComboCenarios(const ent::TabuleiroProto& tabuleiro, QComboBox* combo) {
  ...
  std::vector<std::string> arquivos = arq::ConteudoDiretorio(arq::TIPO_TABULEIRO_ESTATICO);
  for (const auto& arquivo : arquivos) {
    std::string descricao = absl::StrFormat("arquivo: %s", arquivo.c_str());
    combo->addItem(QString::fromUtf8(descricao.c_str()), QVariant(QString::fromUtf8(arquivo.c_str())));
  }
  ExpandeComboBox(combo);
}

std::optional<int> IdCenarioComboCenarios(const QComboBox* combo) {
  QVariant qval = combo->itemData(combo->currentIndex());
  if (!qval.isValid()) return std::nullopt;
  bool ok = false;
  int ret = qval.toInt(&ok);
  return ok ? std::make_optional(ret) : std::nullopt;
}

std::optional<std::string> NomeArquivoCenarioComboCenarios(const QComboBox* combo) {
  QVariant qval = combo->itemData(combo->currentIndex());
  if (!qval.isValid()) return std::nullopt;
  QString ret = qval.toString();
...
void ConfiguraArmasArmaduraOuEscudo(
    ...
    QPushButton* botao_adicionar, QPushButton* botao_duplicar, QCheckBox* checkbox_op, QSpinBox* spin_bonus, QSpinBox* spin_bonus_secundario, QPushButton* botao_remover, QPushButton* botao_ordenar, QPushButton* botao_doar,
    const ent::EntidadeProto& proto, ent::EntidadeProto* proto_retornado, ntf::CentralNotificacoes* central) {
  ...
  if (spin_bonus_secundario != nullptr) {
    lambda_connect(spin_bonus_secundario, SIGNAL(valueChanged(int)), [spin_bonus_secundario, &tabelas, &gerador, tipo, lista, proto_retornado, f_atualiza_ui, tipo_terreno]() {
      const int indice = lista->currentRow();
      auto* aae = BuscaArmasArmadurasEscudos(tipo, proto_retornado);
      if (indice < 0 || indice >= aae->size()) {
        return;
      }
      aae->Mutable(indice)->set_bonus_magico_secundario(spin_bonus_secundario->value());
      ent::RecomputaDependencias(tabelas, tipo_terreno, proto_retornado);
      AtualizaUITesouro(tabelas, gerador, *proto_retornado);
      });
  }
...
void PreencheConfiguraTesouro(
  ...
  ConfiguraArmasArmaduraOuEscudo(
      ...
      gerador.checkbox_arma_op, gerador.spin_bonus_arma, gerador.spin_bonus_arma_secundario,
...
void AdicionaOuEditaNivel(const ent::Tabelas& tabelas, ent::TipoTerreno tipo_terreno, ifg::qt::Ui::DialogoEntidade& gerador, ent::EntidadeProto* proto_retornado, bool novo = false) {
  ...
  if (novo) {
    auto* ic = proto_retornado->add_info_classes();
    ic->set_id("outro");
    ic->set_nivel(1);
...
std::unique_ptr<ent::TabuleiroProto> Visualizador3d::AbreDialogoCenario(
    const ntf::Notificacao& notificacao) {
  auto proto_retornado = std::make_unique<ent::TabuleiroProto>();
  ...
  ent::Cor cor_chuva = tab_proto.has_cor_chuva() ? tab_proto.cor_chuva() : ent::CorParaProto(ent::COR_BRANCA);
  gerador.botao_cor_chuva->setStyleSheet(CorParaEstilo(cor_chuva));
  lambda_connect(gerador.botao_cor_chuva, SIGNAL(clicked()), [this, dialogo, &gerador, &cor_chuva] {
        QColor cor = QColorDialog::getColor(ProtoParaCor(cor_chuva), dialogo, QObject::tr("Cor do efeito da chuva"));
        if (!cor.isValid()) {
          return;
        }
        gerador.botao_cor_chuva->setStyleSheet(CorParaEstilo(cor));
        cor_chuva.CopyFrom(CorParaProto(cor));
      });
  ...
  lambda_connect(gerador.botao_clonar, SIGNAL(clicked()), [this, &gerador, &proto_retornado, dialogo] () {
    ...
    if (id_combo.has_value()) {
      ...
      if (*id_combo == id_corrente) return;
    ...
    std::optional<std::string> arquivo_combo = NomeArquivoCenarioComboCenarios(gerador.combo_id_cenario);
    if (arquivo_combo.has_value() && !arquivo_combo->empty()) {
      ent::TabuleiroProto cenario;
      try {
        ntf::Notificacao tabuleiro_salvo;
        arq::LeArquivoBinProto(arq::TIPO_TABULEIRO_ESTATICO, *arquivo_combo, &tabuleiro_salvo);
        cenario = tabuleiro_salvo.tabuleiro();
      } catch (arq::ParseProtoException& ppe) {
        LOG(ERROR) << "erro abrindo arquivo: " << ppe.what();
        dialogo->reject();
        return;
      }
  ...
  lambda_connect(gerador.botoes, SIGNAL(accepted()),
                 [this, tab_proto, dialogo, &gerador, &cor_ambiente_proto, &cor_direcional_proto, &cor_piso_proto, &cor_nevoa_proto, &cor_chuva, &proto_retornado] {
    // Descricao.
    ...
    proto_retornado->mutable_cor_chuva()->Swap(&cor_chuva);
...
std::unique_ptr<ent::OpcoesProto> Visualizador3d::AbreDialogoOpcoes(
    const ntf::Notificacao& notificacao) {
  auto proto_retornado = std::make_unique<ent::OpcoesProto>(notificacao.opcoes());
...
ent::EntidadeProto* Visualizador3d::AbreDialogoTipoForma(const ntf::Notificacao& notificacao) {
  ...
  lambda_connect(dialogo, SIGNAL(accepted()),
    ...
    if (gerador.combo_transicao->currentIndex() == ent::EntidadeProto::TRANS_CENARIO) {
      ...
      int val = 0;
      if (!qval.isValid() || qval.type() != QVariant::Int) {
        // Busca um novo id.
      ...
      if (qval.type() == QVariant::String) {
        proto_retornado->set_nome_arquivo_cenario_transicao(qval.toString().toStdString());
      }
