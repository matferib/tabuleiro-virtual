...
def _gen_ui_header(ctx):
    ...
    ctx.actions.run(
        ...
        arguments = args,
        #executable = ctx.bin_dir.path + '/' + ctx.executable.uic.basename,
        executable = ctx.attr.uic, 
    )
...
gen_ui_header = rule(
    ...
    attrs = {
        ...
        #"uic": attr.label(allow_single_file=True, executable=True, cfg='exec') 
        "uic": attr.string() 
    },
)
...
def qt_ui_library(name, ui, deps, **kwargs):
    ...
    gen_ui_header(
        ...
        uic = select({
            ...
            "@platforms//os:osx": "/opt/homebrew/Cellar/qtbase/6.9.3_1/share/qt/libexec/uic",
        }),
    )
    cc_library(
        name = name,
...
def qt_resource(name, files, **kwargs):
    ...
    gencpp(
        ...
        rcc = select({
            ...
            "@platforms//os:osx": "/opt/homebrew/Cellar/qtbase/6.9.3_1/share/qt/libexec/rcc",
        }),
